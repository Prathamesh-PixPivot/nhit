...
namespace App\Http\Controllers\Backend\Import\Vendor;
...
class VendorController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ipaddrress = request()->getClientIp();
    ...
    public function index(Request $request)
    {
        ...
        $sl_no_filter = Vendor::select('sl_no')->groupBy('sl_no')->get();
        if ($request->ajax()) {
            $data = Vendor::get();
        ...
        // dd($last_query);
        return view('backend.import.vendor.index', compact('sl_no_filter'));
    }
    ...
    {
        //
        return view('backend.import.vendor.import-file');
    }
    ...
    public function store(Request $request)
    {
        ...
        // DB::statement('SET FOREIGN_KEY_CHECKS=1;');
        /* $country_id = $this->helper->getCountry('india', 'name');
        $state_id = $this->helper->getState('Delhi', 'name');
        $city_id = $this->helper->getCity('Delhi', 'name');
        dd($country_id, $state_id, $city_id); */
        try {
            \Excel::import(new VendorsImport, $request->import_vendor);
