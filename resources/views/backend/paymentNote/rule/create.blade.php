                            ...
                            <tbody>
                                @foreach ($approvalSteps as $rule)
                                    @foreach ($rule->approvers->groupBy('approver_level') as $level => $approvers)
                                        <tr>
                                            <td class="border p-3">Approver {{ $level }}</td>
                                            <td class="border p-3">
                                                {{ implode(', ', $approvers->pluck('user.name')->toArray()) }}
                                            </td>
                                            <td class="border p-3">
                                                @if ($rule->max_amount == null)
                                                    Above {{ number_format($rule->min_amount) }}
                                                @else
                                            ...
                                            <td class="border p-3">
                                                @can(['payment-note-edit-rule'])
                                                    <a href="{{ route('backend.payment-note-approval.edit', $rule->id) }}">
                                                        <i class="bi bi-pencil-square"></i>
                                                    </a> |
                                                @endcan
                                                @can(['payment-note-view-rule'])
                                                    <a href="{{ route('backend.payment-note-approval.show', $rule->id) }}">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                @endcan
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById("add-approver").addEventListener("click", function() {
                ...
                <button type="button" class="remove-approver btn btn-primary">X</button>
            `;
