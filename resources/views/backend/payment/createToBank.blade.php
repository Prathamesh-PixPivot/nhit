    ...
    <section class="section">
        <div class="row">
            <div class="col-lg-12">
                                ...
                                <tbody id="table-body-request">
                                    <tr>
                                        <form name="requestFormCreate" method="post" id="requestFormCreate">
                                            ...
                                            <input type="hidden"
                                                class="form-control @error('template_type_hidden') is-invalid @enderror"
                                                id="template_type_hidden" value="Internal" name="internal_external"
                                                readonly>
                                                ...
                                                <div class="col-md-12 p-1">
                                                    ...
                                                    <div class="col-md-12 from_account_dropdown">
                                                        <select id="from_account"
                                                            ...
                                                            name="from_account" data-index="0" tabindex="3">
                                                        </select>
                                                        @error('from_account')
                                                            <span class="text-danger">{{ $message }}</span>
                                                        @enderror
                                            ...
                                            <td>
                                                <div class="col-md-12 p-1">
                                                    ...
                                                    <div class="col-md-12 vendor_code_dropdown">
                                                        <select id="vendor_code"
                                                            ...
                                                            name="vendor_code" data-index="0" tabindex="4">
                                                        </select>
                                                        @error('vendor_code')
                                                            <span class="text-danger">{{ $message }}</span>
                                                        @enderror
                                                ...
                                                <input type="text"
                                                    ...
                                                    id="benificiary_name" value="{{ old('benificiary_name') ?? '' }}"
                                                ...
                                                <input type="text"
                                                    ...
                                                    id="ifsc_code" value="{{ old('ifsc_code') ?? '' }}" name="ifsc_code"
                                                    readonly>
                                            </td>
                ...
                <form action="{{ route('backend.payments.store') }}" method="post" id="requestPaymentForm" name="requestForm">
                    @csrf
                    <div class="list-group mt-5 request-in-queue delete-item">
                        @if (!empty($notes))
                            @include('backend.payment.requestFormBank', [
                                'cartItems' => $notes,
                            ])
                        @endif
                    </div>
                    <div class="col-md-12 mt-5 btnRequestForm {{ empty($notes) ? 'hide' : 'show' }}">
                        ...
                        <div class="form-check mt-3">
                            ...
                            <input type="text" name="shortcut_name" placeholder="Enter shortcut name" id="shortcut_name"
...
@push('script')
    ...
    <script>
        ...
        function projectsDropDown() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('.select-project').append(`<option>Select Options</option>`);
        ...
        $(document).on('change', '.select-project', function() {
            ...
            paymentFromDropDown(postData, index);

        });

        function paymentFromDropDown(postData, index) {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            ...
            $.ajax({
                ...
                success: function(response) {
                    // $('#from_account').val([]);
                    $(`#from_account_dropdown${index}`).html('');
                    from_account_dropdown = ``;
                    from_account_dropdown =
                        `<select id="from_account${index}" class="form-control from_account @error('from_account') is-invalid @enderror" name="vendor[${index}][account_full_name]" data-index="${index}">`;
                    ...
                    $(`#from_account_dropdown${index}`).html(from_account_dropdown);
                    // $('#from_account'). val([]).change(); // Update selected options dynamically
                    $(`#from_account${index}`).select2();
                    ...
                    setTimeout(function() {
                        $('#from_account').attr('tabindex', 3);
                        $('#vendor_code').attr('tabindex', 4);
                    }, 100);
                },
                error: function(xhr, status, error) {
                    console.error('Error fetching options:', error);
        ...
        $(document).on('change', '.from_account', function() {
            ...
            // console.log(`#from_account_no${index}`, 'id');

            $(`#from_account_no${index}`).val(selectedAccount);
            // alert(1);
            var postData = {
                s_no: s_no,
                project: project,
