            ...
            <div class="col-lg-12">
                                ...
                                <tbody id="table-body-request">
                                    <tr>
                                        <form name="requestFormCreate" method="post" id="requestFormCreate">
                                            ...
                                            {{-- <td>
                                                <div class="col-md-12  p-1 left-align">
                ...
                <form class="" action="{{ route('backend.payments.store') }}" method="post"
                    ...
                    <div class="list-group mt-5 request-in-queue delete-item">
                        @if (!empty(\Cache::get('cart_' . $uid)))
                            @include('backend.payment.requestForm', [
                                'cartItems' => \Cache::get('cart_' . $uid),
                            ])
                        @endif
                    </div>
                    <div class="col-md-12 mt-5 btnRequestForm {{ empty(\Cache::get('cart_' . $uid)) ? 'hide' : 'show' }}">
...
@push('script')
    ...
    <script>
        ...
        //      $('#project').next('.select2-container').attr('tabindex', 2);

        //   $(document).on("keydown", function (e) {
        //     if (e.key === "Tab") {
        //                    // Custom logic for Tab navigation
        //       const $focusable = $("[tabindex]").filter(":visible").sort((a, b) => {
        ...
        //       if (nextIndex >= $focusable.length) nextIndex = 0;              // Loop to first
        //       if (nextIndex < 0) nextIndex = $focusable.length - 1;              // Loop to last
